...
def parse_method(args, n, c, d, device, edge_index):  # Add edge_index parameter
    if args.method == 'pmlp_hopgnn': 
        model = PMLP_HOPGNN(
            in_channels=d, 
            ...
            args=args, 
            num_node=n, 
            edge_index=edge_index,  # Use edge_index here
            num_hop=args.num_hop
        ).to(device)
    elif args.method == 'pmlp_gcn': 
        model = PMLP_GCN(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_gcnii': 
        model = PMLP_GCNII(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_jknet': 
        model = PMLP_JKNet(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_sgc': 
        model = PMLP_SGC(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_sgc_res': 
        model = PMLP_SGCres(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_sgc_resinf': 
        model = PMLP_SGCresinf(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_appnp': 
        model = PMLP_APPNP(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_appnp_res': 
        model = PMLP_APPNPres(d, args.hidden_channels, c, args, n).to(device)
    elif args.method == 'pmlp_appnp_resinf': 
        model = PMLP_APPNPresinf(d, args.hidden_channels, c, args, n).to(device)
    else: 
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--num_hop', type=int, default=4, help='number of hops for HOP GNN integration')
