...
### Load model ###
edge_index = dataset.graph['edge_index']  # Get edge_index from dataset
model = parse_method(args, n, c, d, device, edge_index)
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if isinstance(model, PMLP_HOPGNN):
            out = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'], use_conv=False)
        else:
            out = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'], use_conv=args.conv_tr)
        train_start = time.time()

        if isinstance(model, PMLP_HOPGNN):
        # Call forward without use_conv
            out = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'])
        else:
        # Call forward with use_conv for other models
            out = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'], use_conv=args.conv_tr)
        #out = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'], use_conv = args.conv_tr)
        ...
        if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins', 'yelp'):
            ...
            loss = nn.CrossEntropyLoss()(out[train_idx], dataset.label[train_idx].squeeze())
        
        loss.backward()
        optimizer.step()
        run_losses['train'].append(loss.item())

        # Track the loss for training
    ...
    # Append run losses to the main losses dictionary
    all_losses['train'].append(run_losses['train'])
    all_losses['valid'].append(run_losses['valid'])
    all_losses['test'].append(run_losses['test'])
    logger.print_statistics(run)
train_time = sum(train_times) / len(train_times)
# train_mem = sum(train_mems) / len(train_mems)
para_num = count_parameters(model)

# Save losses to a file for later visualization
import pickle
filename = f"losses_{args.dataset}.pkl"
