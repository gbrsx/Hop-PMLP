...
class HopGNN(torch.nn.Module):
    ...
    def reset_parameters(self):
        # Reset each layer’s parameters
        self.fc.reset_parameters()
        self.atten_self.reset_parameters()
        self.atten_neighbor.reset_parameters()
        self.classifier.reset_parameters()
        
        # Reset parameters of layers within lists (if they support resetting)
        for interaction_layer in self.interaction_layers:
            if hasattr(interaction_layer, "reset_parameters"):
                interaction_layer.reset_parameters()
        
        for norm_layer in self.norm_layers:
            if hasattr(norm_layer, "reset_parameters"):
                norm_layer.reset_parameters()

    def forward(self, inputs, single_layer=False):
        # Step 1: Preprocess hop-information
        h = self.build_hop(inputs)
        # Step 2: Hop embedding
        h = self.embedding(h)
        # Step 3: Hop interaction
        if single_layer:
            # Use only the first interaction layer
            h = self.interaction_layers[0](h)
            h = self.activate(h)
        else:
